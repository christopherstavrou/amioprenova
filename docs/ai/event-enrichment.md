...
A generic, reusable guide for AI agents enriching or creating event entries in `src/data/events.json`. Applicable to any event — scraped from Facebook, entered manually, or created from scratch.
...
- Read/write access to `src/data/events.json`
...
1. **Enrich existing** — one or more events are already in `events.json`; fill gaps and improve quality
2. **Create new** — given raw information (title, date, venue, description, flyer, URL); build a complete entry and insert into `events.json` in chronological order
...
- Insert into `events.json` in ascending `startDate` order
...
- EN pages use: `titleEn ?? title`, `descriptionEn ?? description`, `bodyEn ?? body`
- BG pages use: `titleBg ?? title`, `descriptionBg ?? description`, `bodyBg ?? body`
